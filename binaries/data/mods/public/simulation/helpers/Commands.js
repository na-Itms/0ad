...
var g_Commands = {
	...
	"formation": function(player, cmd, data)
	{
		if (cmd.name == "formations/null")
		{
			...
			CreateFormation(data.entities, player, cmd.name);
	},

	"join-formation": function(player, cmd, data)
	{
		let cmpTemplateManager = Engine.QueryInterface(SYSTEM_ENTITY, IID_TemplateManager);
		let formationTemplate = cmpTemplateManager.GetCurrentTemplateName(cmd.target);

		for (let ent of data.entities)
		{
			let cmpIdentity = Engine.QueryInterface(ent, IID_Identity);
			let cmpUnitAI = Engine.QueryInterface(ent, IID_UnitAI);
			if (cmpUnitAI && cmpIdentity && cmpIdentity.CanUseFormation(formationTemplate))
				cmpUnitAI.JoinFormation(cmd.target);
		}
	},
...
/**
 * Creates a formation from all entities that can use the specified one, and
 * return the entity ID of the formation controller.
 */
function CreateFormation(ents, player, formationTemplate)
{
	// Separate out the units that don't support forming
	// Compute the center position for the new formation
	let formableEnts = [];
	let positions = [];
	for (let ent of ents)
	{
		...
		formableEnts.push(ent);
		positions.push(cmpPosition.GetPosition2D());
	}

	if (formableEnts.length == 0)
	{
		warn("No formable entities found in selection");
		return;
	}
	...
	let cmpFormationPosition = Engine.QueryInterface(formationEnt, IID_Position);
	let avgpos = Vector2D.avg(positions);
	cmpFormationPosition.JumpTo(avgpos.x, avgpos.y);

	// Don't make the formation controller show up in range queries
	let cmpRangeManager = Engine.QueryInterface(SYSTEM_ENTITY, IID_RangeManager);
	cmpRangeManager.SetEntityFlag(formationEnt, "normal", false);

	// Now give the formable units the order to join the new formation
	for (let ent of formableEnts)
	{
		let cmpUnitAI = Engine.QueryInterface(ent, IID_UnitAI);
		cmpUnitAI.JoinFormation(formationEnt);
	}
}
