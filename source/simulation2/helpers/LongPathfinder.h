...
public:
	...
	void GetDebugData(u32& steps, double& time, Grid<u8>& grid)
	{
		if (m_UseJPS)
			...
			GetDebugDataTile(steps, time, grid);
	}
...
private:
	 ...
	 * TODO: remove it once we're certain it won't be useful anymore
	 */
	void ComputeTilePath(entity_pos_t x0, entity_pos_t z0, const PathGoal& origGoal, pass_class_t passClass, WaypointPath& path);
	void GetDebugDataTile(u32& steps, double& time, Grid<u8>& grid);
