...
public:
	...
	void test_LoadTemplate_scriptcache()
	{
		...
		ScriptInterface::ToJSVal(rq.cx, &val, &foundation->GetChild("VisualActor"));
...
#define GET_FIRST_ELEMENT(n, templateName) \
		...
		{ \
			if (it->first[0] == '@') \
				continue; \
			ScriptInterface::ToJSVal(rq.cx, &val, it->second); \
			break; \
		}
