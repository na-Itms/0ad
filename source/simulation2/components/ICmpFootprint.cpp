...
JS::Value ICmpFootprint::GetShape_wrapper() const
{
	...
	if (shape == CIRCLE)
	{
		...
		ScriptInterface::ToJSVal<std::string>(rq.cx, &ptype, "circle");
		ScriptInterface::ToJSVal(rq.cx, &pradius, size0);
		ScriptInterface::ToJSVal(rq.cx, &pheight, height);
	...
	else
	{
		...
		JS::RootedValue pheight(rq.cx);
		ScriptInterface::ToJSVal<std::string>(rq.cx, &ptype, "square");
		ScriptInterface::ToJSVal(rq.cx, &pwidth, size0);
		ScriptInterface::ToJSVal(rq.cx, &pdepth, size1);
		ScriptInterface::ToJSVal(rq.cx, &pheight, height);
		JS_SetProperty(rq.cx, obj, "type", ptype);
