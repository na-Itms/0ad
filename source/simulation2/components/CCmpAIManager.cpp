...
public:
		...
		JS::HandleValue position, JS::HandleValue goal, pass_class_t passClass)
	{
		...
		self->m_ScriptInterface->FromJSVal<CFixedVector2D>(rq.cx, position, pos);
		self->m_ScriptInterface->FromJSVal<CFixedVector2D>(rq.cx, goal, goalPos);

		self->ComputePath(pos, goalPos, passClass, waypoints);
		self->m_ScriptInterface->ToJSVal<std::vector<CFixedVector2D> >(rq.cx, &retVal, waypoints);
	...
	bool TryLoadSharedComponent()
	{
		...
		ScriptInterface::CreateObject(rq.cx, &playersID);

		for (size_t i = 0; i < m_Players.size(); ++i)
		{
			JS::RootedValue val(rq.cx);
			m_ScriptInterface->ToJSVal(rq.cx, &val, m_Players[i]->m_Player);
		...
		const std::map<std::string, pass_class_t>& nonPathfindingPassClassMasks, const std::map<std::string, pass_class_t>& pathfindingPassClassMasks)
	{
		...
		ScriptInterface::ToJSVal(rq.cx, &m_PassabilityMapVal, passabilityMap);
		ScriptInterface::ToJSVal(rq.cx, &m_TerritoryMapVal, territoryMap);
		...
		const std::map<std::string, pass_class_t>& nonPathfindingPassClassMasks, const std::map<std::string, pass_class_t>& pathfindingPassClassMasks)
	{
		...
		if (dimensionChange || justDeserialized)
			ScriptInterface::ToJSVal(rq.cx, &m_PassabilityMapVal, m_PassabilityMap);
		else
		{
	...
	void UpdateTerritoryMap(const Grid<u8>& territoryMap)
	{
		...
		ScriptInterface::Request rq(m_ScriptInterface);
		if (dimensionChange)
			ScriptInterface::ToJSVal(rq.cx, &m_TerritoryMapVal, m_TerritoryMap);
		else
		{
			// Avoid a useless memory reallocation followed by a garbage collection.
	...
	void LoadEntityTemplates(const std::vector<std::pair<std::string, const CParamNode*> >& templates)
	{
		...
		ScriptInterface::CreateObject(rq.cx, &m_EntityTemplates);

		JS::RootedValue val(rq.cx);
		for (size_t i = 0; i < templates.size(); ++i)
		{
			templates[i].second->ToJSVal(rq.cx, false, &val);
...
private:
	...
	void LoadPathfinderClasses(JS::HandleValue state)
	{
		...
		ScriptInterface::CreateObject(rq.cx, &classesVal);
