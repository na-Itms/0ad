...
#define RMS_CONTEXT_SIZE 96 * 1024 * 1024
...
void* CMapGeneratorWorker::RunThread(CMapGeneratorWorker* self)
{
	...
	shared_ptr<ScriptContext> mapgenContext = ScriptContext::CreateContext(g_ScriptContext, RMS_CONTEXT_SIZE);
...
JS::Value CMapGeneratorWorker::LoadHeightmap(ScriptInterface::CmptPrivate* pCmptPrivate, const VfsPath& filename)
{
	...
	ToJSVal_vector(rq.cx, &returnValue, heightmap);
	return returnValue;
}
...
JS::Value CMapGeneratorWorker::LoadMapTerrain(ScriptInterface::CmptPrivate* pCmptPrivate, const VfsPath& filename)
{
	...
	{
		JS_ReportErrorUTF8(rq.cx, "Terrain file \"%s\" does not exist!", filename.string8().c_str());
		return JS::UndefinedValue();
	}
	...
	{
		JS_ReportErrorUTF8(rq.cx, "Could not load terrain file \"%s\" too old version!", filename.string8().c_str());
		return JS::UndefinedValue();
	}
