...
bool deepfreeze(JSContext* cx, uint argc, JS::Value* vp)
{
	...
	{
		JSAutoRequest rq(cx);
		JS_ReportErrorUTF8(cx, "deepfreeze requires exactly one object as an argument.");
		return false;
	}
...
bool ScriptInterface::SetGlobal_(const char* name, JS::HandleValue value, bool replace, bool constant, bool enumerate)
{
	...
	if (found)
	{
		...
		if (!desc.writable())
		{
			if (!replace)
			{
				JS_ReportErrorUTF8(rq.cx, "SetGlobal \"%s\" called multiple times", name);
				return false;
			}
			...
			{
				JS_ReportErrorUTF8(rq.cx, "The global \"%s\" is permanent and cannot be hotloaded", name);
				return false;
			}
...
bool ScriptInterface::EnumeratePropertyNames(JS::HandleValue objVal, bool enumerableOnly, std::vector<std::string>& out) const
{
	...
	for (size_t i = 0; i < props.length(); ++i)
	{
		...
		std::string propName;
		if (!FromJSVal(rq.cx, val, propName))
			return false;
