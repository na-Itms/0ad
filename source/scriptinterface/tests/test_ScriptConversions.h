...
class TestScriptConversions : public CxxTest::TestSuite
{
	...
	void convert_to(const T& value, const std::string& expected)
	{
		...
		ScriptInterface::ToJSVal(rq.cx, &v1, value);
	...
	void roundtrip(const T& value, const char* expected)
	{
		...
		T v2 = T();
		TS_ASSERT(ScriptInterface::FromJSVal(rq.cx, v1, v2));
	...
	void call_prototype_function(const T& u, const T& v, const std::string& func, const std::string& expected)
	{
		...
		JS::RootedValue v1(rq.cx);
		ScriptInterface::ToJSVal(rq.cx, &v1, v);
		JS::RootedValue u1(rq.cx);
		ScriptInterface::ToJSVal(rq.cx, &u1, u);

		T r;
		JS::RootedValue r1(rq.cx);

		TS_ASSERT(script.CallFunction(u1, func.c_str(), r, v1));
		ScriptInterface::ToJSVal(rq.cx, &r1, r);
...
public:
	...
	void test_integers()
	{
		...
		ScriptInterface::ToJSVal<i32>(rq.cx, &val0, 0);
		ScriptInterface::ToJSVal<i32>(rq.cx, &val1, JSVAL_INT_MAX - 1);
		ScriptInterface::ToJSVal<i32>(rq.cx, &val2, JSVAL_INT_MAX);
		ScriptInterface::ToJSVal<i32>(rq.cx, &val3, JSVAL_INT_MIN + 1);
		ScriptInterface::ToJSVal<i32>(rq.cx, &val4, -(i64)2147483648u); // JSVAL_INT_MIN
		...
		ScriptInterface::ToJSVal<u32>(rq.cx, &val5, 0);
		ScriptInterface::ToJSVal<u32>(rq.cx, &val6, 2147483646u); // JSVAL_INT_MAX-1
		ScriptInterface::ToJSVal<u32>(rq.cx, &val7, 2147483647u); // JSVAL_INT_MAX
		ScriptInterface::ToJSVal<u32>(rq.cx, &val8, 2147483648u); // JSVAL_INT_MAX+1
	...
	void test_nonfinite()
	{
		...
		ScriptInterface::ToJSVal(rq.cx, &testNANVal, NAN);
		TS_ASSERT(ScriptInterface::FromJSVal(rq.cx, testNANVal, f));
		TS_ASSERT(isnan(f));
	}
	...
	void test_utf8utf16_conversion()
	{
		...
		JS::RootedValue v1(rq.cx);
		ScriptInterface::ToJSVal(rq.cx, &v1, in_utf8);
		std::wstring test_out_utf16;
		TS_ASSERT(ScriptInterface::FromJSVal(rq.cx, v1, test_out_utf16));
		TS_ASSERT_EQUALS(test_out_utf16, in_utf16);

		JS::RootedValue v2(rq.cx);
		ScriptInterface::ToJSVal(rq.cx, &v2, in_utf16);
		std::string test_out_utf8;
		TS_ASSERT(ScriptInterface::FromJSVal(rq.cx, v2, test_out_utf8));
